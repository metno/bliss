...
corens_selensemble <- function( argv, fg_env, env, plot=F, dir_plot=NA) {
#
# output
...
#------------------------------------------------------------------------------
  ...
  cat( "-- corens selection of ensemble members --\n")
...
#  # select only the k fields with the highest scores
  if (argv$corens_selens_mode == "identity") {
    ixs <- 1:env$k_dim
  } else {
